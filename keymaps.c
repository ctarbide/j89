		...
		if (km_getkey(km, key) != NULL) {
			...
			if (nused == km->u.sparse.nalloc) {
				...
				if (nused >= SPARSE_LIMIT) {
					...
					int tmp_i = km->u.sparse.nalloc + KEYMAP_QUANTUM;
					if (tmp_i < 0 || tmp_i > SHRT_MAX) {
						fprintf(stderr, "fatal: %s:%d: tmp_i < 0 || tmp_i > SHRT_MAX\n", __FILE__, __LINE__);
						exit(1);
					}
					km->u.sparse.nalloc = (short)tmp_i;
					/* km->u.sparse.nalloc guaranteed within 0 and SHRT_MAX */
					b = (struct keybinding *) erealloc(
						(UnivPtr) km->u.sparse.bindings,
						(unsigned short)km->u.sparse.nalloc * sizeof(struct keybinding));
					km->u.sparse.bindings = b;
				}
			}
...
private void
DoLBind(const data_obj *(*findproc) ptrproto((const char *)))
{
	if (curbuf->b_map == NULL) {
...
private char *
fb_aux(const data_obj *cp, struct keymap *map, char *prefix, char *buf, size_t room)
{
	char	*bufp = buf;
...
private void
find_binds(const data_obj *dp, char *buf, size_t size)
{
	char	*endp;
	UnmarkMaps();
	buf[0] = '\0';
	...
	if (endp < buf) {
		fprintf(stderr, "fatal: %s:%d: endp < buf\n", __FILE__, __LINE__);
		exit(1);
	}
	/* endp guaranteed greater or equal than buf */
	endp = fb_aux(dp, curbuf->b_map, "Local:", endp, size - (size_t)(endp - buf));
#ifdef IPROCS
	if (endp < buf) {
		fprintf(stderr, "fatal: %s:%d: endp < buf\n", __FILE__, __LINE__);
		exit(1);
	...
	/* endp guaranteed greater or equal than buf */
	endp = fb_aux(dp, procsmap, "Proc:", endp, size - (size_t)(endp - buf));
#endif
