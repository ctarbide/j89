...
int
LinesToAsInt(LinePtr from, LinePtr to)
{
	long n = LinesTo(from, to);
	if (n < INT_MIN || n > INT_MAX) {
		fprintf(stderr, "fatal: %s:%d: n < INT_MIN || n > INT_MAX\n", __FILE__, __LINE__);
		exit(1);
	}
	return (int)n;
}
...
length(LinePtr line)
{
	return (int)strlen(lcontents(line));
}
...
#				define JENV_INCR	5
				INTPTR_T	diff = p - e;
				size_t		sz = ((size_t)diff + 1) * sizeof(char *);
				const char	**ne;

				ne = malloc(sz + JENV_INCR * sizeof(char *));

				if (ne == NULL) {
					fprintf(stderr, "warning: %s:%d: malloc failed\n", __FILE__, __LINE__);
